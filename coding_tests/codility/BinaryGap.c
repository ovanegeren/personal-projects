...
int solution(int N){
    int arr[32];                    //input range up to 2.147 billion
    int decimal = N;

    // call dec2bin
    // flip array ???

    //iterate through binary array, finding the binary gap
}


int dec2bin(int N, int *binarray, int len){
    int decvalue = N;
    int remainder;
    
    int i = 0;
    while(decvalue > 1 && i < len){
